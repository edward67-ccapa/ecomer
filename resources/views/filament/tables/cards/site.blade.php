...
<div class="flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
    ...
    <div class="flex flex-1 flex-col p-4">
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            @if ($record->dominio)
                · {{ $record->dominio->nombre }}/{{ $record->plantilla?->secciones->reject(fn($s) => strtolower($s->slug) === 'nav')->first()?->slug ?? 'Inicio' }}
            @endif
        </p>

        @if ($record->estado === 'publicado' && $record->dominio)
            <a
                href="/{{ $record->dominio->nombre }}/{{ $record->plantilla?->secciones->reject(fn($s) => strtolower($s->slug) === 'nav')->first()?->slug ?? 'Inicio' }}"
                target="_blank"
