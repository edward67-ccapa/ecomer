...
export default function ServiciosSection({ seccionData, serviciosSitio = [], dominio, siteSlug }) {
    ...
    const itemsFromDetallado = Array.isArray(servicioDetalladoRaw)
        ? servicioDetalladoRaw.map((item, idx) => {
            const nombre = item.nombreServicio || item.titulo || item.Titulo || `Servicio ${idx + 1}`;
            const sub = item.subtitulo || item.Titulo || '';
            const desc = item.Descripcion || item.descripcion || '';
            const img = Array.isArray(item.ImagenesServicios) && item.ImagenesServicios.length > 0
                ? item.ImagenesServicios[0]
                : (typeof item.ImagenesServicios === 'string' ? item.ImagenesServicios : (item.imagen || null));
            const icon = item.icono || icono || null;

            return {
                id: item.id || idx,
                titulo: nombre,
                nombreServicio: item.nombreServicio || nombre,
                subtitulo: sub,
                descripcion: desc,
                imagen: img,
    ...
    const itemsFromSitio = serviciosSitio
        .filter((grupo) => grupo.activo !== false)
        .flatMap((grupo) => grupo.servicios ?? []);

    const todosLosItems = itemsFromDetallado.length > 0 ? itemsFromDetallado : itemsFromSitio;
                                ...
                                <motion.div key={item.id ?? idx} variants={cardVariants}>
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex flex-col items-center justify-center p-6 text-center">
                                            ...
                                            {item.subtitulo && (
                                                <p
                                                    className="text-sm text-white/80 mt-1 text-center line-clamp-2"
                                                    style={{ fontFamily: 'var(--tipografia-texto)' }}
                                                >
                                                    {item.subtitulo}
                                                </p>
